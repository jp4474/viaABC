...
class FineTuneLightning(L.LightningModule):
    def __init__(self, model: nn.Module, lr: float = 1e-4, num_parameters: int = 2):
        super().__init__()
        ...
        self.linear_layer = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(self.latent_dim//2, num_parameters)
        )
    ...
    def training_step(self, batch):
        self.model.train()
        parameters, simulations = batch
        logits = self.get_latent(simulations, mask_ratio=0)
    ...
    def forward(self, inputs):
        logits = self.get_latent(inputs, mask_ratio=0)
        return self.linear_layer(logits)
    
    def get_latent(self, x, mask_ratio=0):
        return self.model.get_latent(x, mask_ratio).mean(dim=1)