    ...
    for ed in "${eds[@]}"; do
        for dec_depth in "${dec_depths[@]}"; do
            for noise in "${noise_factors[@]}"; do
                ...
                if (( $(bc -l <<< "$beta == 0") )); then
                    dirpath="lotka_d${d}_ed${ed}_${enc_depth}_${num_heads}_${dec_depth}_${decoder_num_heads}_ae_mask_${mask}_noise_${noise}"
                else
                    dirpath="lotka_d${d}_ed${ed}_${enc_depth}_${num_heads}_${dec_depth}_${decoder_num_heads}_vae_mask_${mask}_beta_${beta}_noise_${noise}"
                fi
