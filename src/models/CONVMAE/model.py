...
from src.models.components import get_2d_sincos_pos_embed
...
#        torch.nn.init.normal_(self.cls_token, std=.02)
    ...
    def patchify(self, imgs):
        """
        imgs: [N, 1, H, W]  integer class map with values 0–5
        Returns: [N, L] patch labels
        """
        p = 16
        ...
        # reduce each patch to a single label
        # example: use the center pixel or majority vote
        patch_labels = x[:, :, :, p//2, :, p//2]   # [N, 1, h, w]

        return patch_labels.reshape(N, h*w)       # [N, L]
...
#        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))
    ...
    def forward_loss(self, imgs, pred, mask):
        """
        imgs: [N, C, H, W]
        pred: [N, L, 6]        # class logits per patch
        mask: [N, L]           # 0 = keep patch, 1 = masked patch
        """

        # ---- 1. Convert images to patch-level class labels ----
        # This must output [N, L] integer class labels (0–5)
        target = self.patchify(imgs)   # <-- YOU must modify patchify() to output class labels

        # Ensure target is long type for CE loss
        target = target.long()         # [N, L]

        # ---- 2. Compute CE loss per patch ----
        # per-patch CE loss → [N, L]
        loss = F.cross_entropy(
            pred.reshape(-1, pred.size(-1)),    # [N*L, 6]
            target.reshape(-1),                 # [N*L]
            reduction='none'
        ).reshape(pred.size(0), pred.size(1))   # reshape back to [N, L]

        # ---- 3. Only compute loss on masked (removed) patches ----
