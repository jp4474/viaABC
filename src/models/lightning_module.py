...
class PreTrainLightning(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train/space_loss", space_loss, on_step=True, on_epoch=True)
        self.log("train/scaled_space_loss", scaled_space_loss, on_step=True, on_epoch=True)
        self.log("train/loss", total_loss, on_step=True, on_epoch=True)

        if self.annealing:
            self.log("train/kld_weight", self.annealer._slope(), on_step=True)
            self.annealer.step()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val/space_loss", space_loss, on_epoch=True)
        self.log("val/scaled_space_loss", scaled_space_loss, on_epoch=True)
